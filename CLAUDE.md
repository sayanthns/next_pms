...
- **Working-hours settings now in UI** (May 31) — `working_hours_per_day` + `weekly_summary_recipient` exposed in the **Team → AI Settings** tab (`settings.py` get/save + `TeamView.vue`). Previously only in the desk doctype.

- **Sales Order mandatory + financials** (May 31) — `sales_order` Link → ERPNext **Sales Order**, `reqd:1`, enforced on **new projects only** (`validate_sales_order`, grandfather existing). `CreateProjectModal.vue` has a required SO picker; `crud.create_project` accepts `sales_order`. New `get_project_financials(project)` (`project_report.py`) → `{so_value (SO grand_total), budget, actual, budget_util, so_util}`, surfaced in the project status email + multi-project report (Jinja templates `templates/emails/project_*_report.html`) + project dashboard card (`ProjectDashboard.vue`, finance-gated).

- **95% budget guard + approval** (May 31) — `PMS Time Log.before_insert` blocks **new** time entries (timer start OR manual) when project `budget_utilization >= 95` (stopping/updating a running log is an UPDATE → allowed). `next_pms.api.budget.request_budget_increase(project)` (access-checked) emails **sayanth@enfono.in** with figures; `store/timer.js` catches the Budget-Exhausted error and offers to send the request. **Unblock is automatic** once sayanth raises `total_budget` (utilisation drops < 95%). No override doctype.
